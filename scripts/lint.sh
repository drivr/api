...
poetry run autoflake \
    ...
    --in-place $APP_PATH $UNIT_TEST_PATH $INTEGRATION_TEST_PATH
...
poetry run isort $APP_PATH $UNIT_TEST_PATH $INTEGRATION_TEST_PATH

# Audit python files on app and unit tests.
poetry run pylama $APP_PATH $UNIT_TEST_PATH $INTEGRATION_TEST_PATH
...
poetry run black --check $APP_PATH $UNIT_TEST_PATH $INTEGRATION_TEST_PATH
